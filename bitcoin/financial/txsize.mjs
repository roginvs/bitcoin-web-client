import { packTx } from "../protocol/messages.create.mjs";
import { readTx } from "../protocol/messages.parse.mjs";
import { describe, eq } from "../tests.mjs";
import { parseHexToBuf } from "../utils/arraybuffer-hex.mjs";

/**
 * @param {import("../protocol/messages.types").TransactionPayload} txFullRaw
 */
export function getTxSize(txFullRaw) {
  const txParsed = readTx(txFullRaw)[0];
  const txNoWitness = packTx({
    ...txParsed,
    isWitness: false,
  });
  const size = txParsed.isWitness
    ? txFullRaw.byteLength
    : txNoWitness.byteLength;
  const weight =
    txNoWitness.byteLength * 4 +
    (txFullRaw.byteLength - txNoWitness.byteLength);

  return {
    size,
    weight,
    // noWitnessSize: txNoWitness.length,
    vbytes: Math.ceil(weight / 4),
  };
}

describe(`Get tx size`, () => {
  eq(
    getTxSize(
      /** @type {import("../protocol/messages.types").TransactionPayload} */
      (
        parseHexToBuf(
          ""
        )
      )
    ),
    {
      //size is demoTx.length,
      size: 410,
      weight: 1310,
      vbytes: Math.ceil(327.5),
    },
    `Demo tx`
  );

  eq(
    getTxSize(
      /** @type {import("../protocol/messages.types").TransactionPayload} */
      (
        parseHexToBuf(
          `
            b20af3425990ac00000000`
            .split("\n")
            .join("")
            .replace(/\s+/g, "")
        )
      )
    ),
    {
      //size is demoTx.length,
      size: 218,
      weight: 542,
      vbytes: Math.ceil(135.5),
    },
    `Demo tx 2`
  );
});
